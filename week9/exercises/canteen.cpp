...
void testcase() {
    ...
    for (int i = 0; i < n; i++){
        int day, cost;
        std::cin >> day >> cost;
        eaG.addEdge(src, i, day, 0);
        eaG.addEdge(i, n + i, day, cost);
    }
    ...
    for (int i = 0; i < n; i++){
        ...
        std::cin >> stud >> cost;
        students += stud;
        eaG.addEdge(n + i, sink, stud, (-1) * cost);
    }
    ...
    for (int i = 0; i < n - 1; i++){
        int volume, cost;
        std::cin >> volume >> cost;
        eaG.addEdge(n + i, n + i + 1, volume, cost);
    }
    ...
    if(flow < students)
        ...
        std::cout << "possible ";
	std::cout << flow << " " << (-1) * cost << std::endl; // for 80pts -> (-1) * cost
}
